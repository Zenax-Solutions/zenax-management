...
<div class="flex flex-wrap">
    <x-inputs.group class="w-full">
        <x-inputs.text name="name" label="Name" :value="old('name', $editing ? $facebookAd->name : '')" maxlength="255" placeholder="Name" required>
        </x-inputs.text>
    </x-inputs.group>

    <x-inputs.group class="w-full">
        <x-inputs.textarea name="content" label="Content" required>
            {{ old('content', $editing ? $facebookAd->content : '') }}</x-inputs.textarea>
    </x-inputs.group>
    ...
    <x-inputs.group class="w-full">
        <x-inputs.number name="reach" label="Reach" :value="old('reach', $editing ? $facebookAd->reach : '')" placeholder="Reach">
        </x-inputs.number>
    </x-inputs.group>

    <x-inputs.group class="w-full">
        <x-inputs.number name="leads" label="Leads" :value="old('leads', $editing ? $facebookAd->leads : '')" placeholder="Leads">
        </x-inputs.number>
    </x-inputs.group>

    <x-inputs.group class="w-full">
        <x-inputs.number name="cost" label="Cost" :value="old('cost', $editing ? $facebookAd->cost : '')" placeholder="Cost" required></x-inputs.number>
    </x-inputs.group>
</div>
