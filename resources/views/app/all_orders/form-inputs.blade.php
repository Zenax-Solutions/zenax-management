...
<div class="flex flex-wrap">
    ...
    <x-inputs.group class="w-full">
        <x-inputs.number name="discount" label="Discount" :value="old('discount', $editing ? $orders->discount : '')" max="255" placeholder="Discount">
        </x-inputs.number>
    </x-inputs.group>

    <x-inputs.group class="w-full">
        <x-inputs.text name="total" label="Total" :value="old('total', $editing ? $orders->total : '')" maxlength="255" placeholder="Total" required>
        </x-inputs.text>
    </x-inputs.group>
    ...
    <x-inputs.group class="w-full">
        <x-inputs.date name="end_date" label="End Date"
            value="{{ old('end_date', $editing ? optional($orders->end_date)->format('Y-m-d') : '') }}" max="255"
    ...
    <x-inputs.group class="w-full">
        <x-inputs.partials.label name="payment_proof" label="Payment Proof"></x-inputs.partials.label><br />

        <input type="file" name="payment_proof" id="payment_proof" class="form-control-file" />

        @if ($editing && $orders->payment_proof)
            <div class="mt-2">
                <a href="{{ \Storage::url($orders->payment_proof) }}" target="_blank"><i
                        class="icon ion-md-download"></i>&nbsp;Download</a>
            </div>
        @endif @error('payment_proof')
